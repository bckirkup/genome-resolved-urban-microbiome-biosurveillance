...
CHECKM2DB="/srv/lustre01/project/mmrd-cp3fk69sfrq/user/Shotgun-metagenomics/checkm2_databases/CheckM2_database/"
GTDBTK_DB="/srv/lustre01/project/mmrd-cp3fk69sfrq/user/Shotgun-metagenomics/gtbk_db_new/release226/"
...
# Output dirs
GLOBAL_OUT="$BASE_PATH/Environmental_metagenomics_global_derep"
...
mkdir -p "$ALL_GENOMES" "$GTDB_OUT" "$COVERM_OUT"

# Runtime log
RUNTIME_LOG="$GLOBAL_OUT/runtime_metrics_global1.csv"
...
############################################
# Step 1: Collect HQ MAGs across all projects
############################################
...
for PROJECT in ambulance hosp_env hosp_sewage public_transport; do
  ...
  for SAMPLE in $(cat "$SRA_PATH/SRA_ids"); do
    ...
    if [[ -f "$CHECKM_TSV" ]]; then
      ...
      awk -F'\t' 'NR>1 && $2 >= 80 && $3 <= 10 {print $1}' "$CHECKM_TSV" | while read bin; do
        src="$BIN_DIR/${bin}.fa"
...
#step_start=$(date +%s)
#gtdbtk classify_wf --genome_dir "$REP_GENOMES" --out_dir "$GTDB_OUT" \
 # --cpus "$THREADS" --extension fa --skip_ani_screen 
#step_end=$(date +%s)
...
############################################
# Step 4: CoverM abundance profiling
############################################
echo "[Step 4] Running CoverM on all samples"
conda activate coverm || exit 1
for PROJECT in hosp_sewage public_transport; do
  SRA_PATH="$BASE_PATH/$PROJECT"
  for SAMPLE in $(cat "$SRA_PATH/SRA_ids1"); do
    ...
    echo "CoverM finished for $SAMPLE in $((step_end - step_start)) seconds"
  done
done

#echo "Global dereplication, taxonomy, and abundance profiling complete."
